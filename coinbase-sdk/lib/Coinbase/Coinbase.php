...
class Coinbase
{
    ...
    public function __construct($authentication, $tokens=null, $apiKeySecret=null)
    {
        ...
        if (get_config('enrol_bitcoin', 'checkproductionmode') == 1) {
            // if production mode set
            define('API_BASE','https://api.coinbase.com/v1/');
        } else {
            // if sandbox mode set 
            define('API_BASE','https://api.sandbox.coinbase.com/v1/');
    ...
    public function createButtonWithOptions($options=array())
    {
        ...
        if (get_config('enrol_bitcoin','checkproductionmode') == 1){
            // for production mode remove data-env=\"sandbox\" from the embeded div
            ...
            // for sandbox mode add data-env=\"sandbox\" to the embeded div
            $returnValue->embedHtml = "<div class=\"coinbase-button\" data-code=\"" . $response->button->code . "\" data-env=\"sandbox\"></div><script src=\"https://coinbase.com/assets/button.js\" type=\"text/javascript\"></script>";
        }
